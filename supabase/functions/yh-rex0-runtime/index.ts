...
async function dbInsert(
  table: string,
  body: RecordJson,
): Promise<{ rows: RecordJson[]; status: number; error?: string }> {
...
function inScope(principalRef: string, scopes: unknown): boolean {
  return Array.isArray(scopes) && scopes.includes(principalRef);
}
...
function requiredRoles(actionClass: string): string[] {
  ...
  if (actionClass === "CLINICAL" || actionClass === "DESTRUCTIVE") {
    return ["steward", "principal", "clinician"];
  }
...
function authorityError(contract: RecordJson): string | null {
  ...
  const roles = new Set(
    Array.isArray(contract.approval_receipts)
      ? contract.approval_receipts
        .filter((x: any) => x && typeof x.actor_role === "string")
        .map((x: any) => x.actor_role)
      : [],
  );
...
function basicContractError(contract: RecordJson): string | null {
  ...
  if (contract.schema_version !== "action-contract-v1") return "invalid_schema_version";
  ...
  if (!Number.isFinite(issued)) return "invalid_issued_at";
  if (!Number.isFinite(expires)) return "invalid_expires_at";
  if (issued > Date.now() + CLOCK_SKEW_MS) return "issued_at_in_future";
  if (expires <= Date.now()) return "contract_expired";
  if (expires <= issued) return "invalid_contract_window";
...
async function loadIssuer(issuerId: string): Promise<RecordJson | null> {
  const rows = await dbGet(
    `yh_rex0_contract_issuers?issuer_id=eq.${encodeURIComponent(issuerId)}&status=eq.active&select=*`,
  );
  return rows[0] || null;
}
...
async function loadWorker(workerId: string): Promise<RecordJson | null> {
  const rows = await dbGet(
    `yh_rex0_workers?worker_id=eq.${encodeURIComponent(workerId)}&status=eq.active&select=*`,
  );
  return rows[0] || null;
}
...
async function admitContract(input: RecordJson): Promise<Response> {
  ...
  if (nonce.length < 8) return reply({ ok: false, error: "invalid_contract_nonce" }, 400);

  const issuer = await loadIssuer(issuer_id);
  if (!issuer) return reply({ ok: false, error: "issuer_not_active" }, 401);
  if (!inScope(contract?.principal_ref, issuer.principal_refs)) {
    return reply({ ok: false, error: "issuer_principal_scope_denied" }, 403);
  }
  ...
  if (inserted.status === 409) {
    return reply({ ok: false, error: "contract_replay_or_idempotency_conflict" }, 409);
  }
...
async function authenticateWorker(
  input: RecordJson,
  expectedOp: string,
): Promise<{ worker: RecordJson; signed: RecordJson } | Response> {
  ...
  if (nonce.length < 8) return reply({ ok: false, error: "invalid_worker_nonce" }, 400);
...
async function workerPull(input: RecordJson): Promise<Response> {
  ...
  if (!Array.isArray(rows) || rows.length === 0) {
    return reply({ ok: true, status: "NO_WORK" });
  }
...
async function workerReceipt(input: RecordJson): Promise<Response> {
  ...
  const leaseToken = input?.body?.lease_token;
  if (!receipt || typeof receipt !== "object") return reply({ ok: false, error: "missing_receipt" }, 400);
  if (typeof leaseToken !== "string" || leaseToken.length < 8) {
    return reply({ ok: false, error: "missing_lease_token" }, 400);
  }
  if (receipt.lease_token !== leaseToken) return reply({ ok: false, error: "receipt_lease_envelope_mismatch" }, 403);
  if (receipt.principal_ref !== input.principal_ref) {
    return reply({ ok: false, error: "receipt_principal_envelope_mismatch" }, 403);
  }
  ...
  try {
    ...
    return reply({
      ...
      contract_state: contract?.state || "completed",
    });
  } catch (error) {
    return reply({
      ok: false,
...
async function workerFail(input: RecordJson): Promise<Response> {
  ...
  if (typeof body.lease_token !== "string" || body.lease_token.length < 8) {
    return reply({ ok: false, error: "missing_lease_token" }, 400);
  }
  try {
    ...
    return reply({
      ok: true,
      status: contract?.state || "retry_wait",
...
async function workerSweep(input: RecordJson): Promise<Response> {
  ...
  if (auth instanceof Response) return auth;
  const principal = String(input.principal_ref || "");
  if (!principal) return reply({ ok: false, error: "missing_principal_ref" }, 400);
  const result = await rpc("yh_rex0_sweep_expired_leases_for_principal", {
    p_principal_ref: principal,
  });
  return reply({ ok: true, status: "SWEEP_COMPLETE", principal_ref: principal, result });
}
...
async function workerHeartbeat(input: RecordJson): Promise<Response> {
  ...
  if (auth instanceof Response) return auth;
  await dbPatch(
    `yh_rex0_workers?worker_id=eq.${encodeURIComponent(input.worker_id)}`,
